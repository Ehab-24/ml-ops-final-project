...
class Submission(models.Model):
    ...
    def auto_check(self, assignment: Assignment) -> float | None:
        ...
        if not task_mime_type: task_mime_type = "application/octet-stream"
        if not solution_mime_type: solution_mime_type = "application/octet-stream"
        if not submission_mime_type: submission_mime_type = "application/octet-stream"
        ...
        try:
            ...
            gemini_output_text = response_json.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "")
            ...
            if gemini_output_text.startswith(score_prefix):
                try:
                    ...
                    print(f"Error parsing numerical score from Gemini output '{gemini_output_text}': {e}")
                    return None
            else:
                print(f"Gemini output did not start with expected 'Score: ' prefix. Output: '{gemini_output_text}'")
                ...
                if match:
                    try:
                        ...
                        print(f"Extracted score '{extracted_score}' from non-standard output.")
                        return max(0.0, min(extracted_score, float(assignment.max_score)))
                    except ValueError:
            ...
            print(f"Error parsing Gemini API response structure: {e}. Full response: {check_response.text if 'check_response' in locals() else 'No response object'}")
            return None
